...
void OnStartup()
{
	@autoreleasepool
	{
		NSBundle *thisBundle = [NSBundle bundleWithIdentifier:@"com.4D.4DPlugin.SANE"];
		if(thisBundle)
		{
...
// --- SANE
		...
		case 1 :
			SANE_SCANNERS_LIST(pResult, pParams);
			break;

		case 2 :
			SANE_Scan(pResult, pParams);
			break;

		case 3 :
			SANE_SCAN_OPTION_VALUES(pResult, pParams);
			break;

		case 4 :
			SANE_SET_SCAN_OPTION(pResult, pParams);
			break;

		case 5 :
			SANE_Get_scan_option(pResult, pParams);
			break;
	...
	if(SANE::isReady)
	{
		...
		if(SANE_STATUS_GOOD == sane_get_devices(&device_list, SANE_FALSE))
		{
			for(unsigned int i = 0; device_list[i]; ++i)
			{
				...
				json_set_text(json_scanner, L"name", (char *)device_list_item->name);
				json_set_text(json_scanner, L"vendor", (char *)device_list_item->vendor);
				json_set_text(json_scanner, L"model", (char *)device_list_item->model);
				json_set_text(json_scanner, L"type", (char *)device_list_item->type);
	...
	if(device)
	{
		...
		do {
			...
			if(option && option_index)
			{
				...
				json_set_text(json_scanner_option, L"name", (char *)option->name);
				json_set_text(json_scanner_option, L"title", (char *)option->title);
				json_set_i(json_scanner_option, L"option", option_index);
				json_set_i(json_scanner_option, L"type", option->type);
				
				JSONNODE *json_scanner_flags = json_new(JSON_NODE);
				json_set_bool(json_scanner_flags, L"automatic", (option->cap & SANE_CAP_AUTOMATIC) != 0);
				json_set_bool(json_scanner_flags, L"selectable", (option->cap & SANE_CAP_SOFT_SELECT) != 0);
				json_set_bool(json_scanner_flags, L"detectable", (option->cap & SANE_CAP_SOFT_DETECT) != 0);
				json_set_name(json_scanner_flags, L"flags");
				json_push_back(json_scanner_option, json_scanner_flags);
				...
				switch (option->constraint_type)
				{
					...
					{
						JSONNODE *json_constraint_string_list = json_new(JSON_ARRAY);
						const SANE_String_Const *sl = option->constraint.string_list;
						do {
							if(*sl)
							{
								json_set_text(json_constraint_string_list, (char *)*sl++);
							}
							
						}while(*sl);
						
						json_set_name(json_constraint_string_list, L"values");
						json_push_back(json_scanner_option, json_constraint_string_list);
					}
						break;
					case SANE_CONSTRAINT_WORD_LIST:
					{
						JSONNODE *json_constraint_word_list = json_new(JSON_ARRAY);
						const SANE_Word *wl = option->constraint.word_list;
						do {
							if(*wl)
							{
								JSONNODE *e = json_new(JSON_NUMBER);
								json_set_i(e, *wl++);
								json_push_back(json_constraint_word_list, e);
							}
							
						}while(*wl);
						
						json_set_name(json_constraint_word_list, L"values");
						json_push_back(json_scanner_option, json_constraint_word_list);
					}
						break;
					case SANE_CONSTRAINT_RANGE:
					{
						...
						JSONNODE *json_constraint_range = json_new(JSON_NODE);
						json_set_i(json_constraint_range, L"min", range->min);
						json_set_i(json_constraint_range, L"max", range->max);
						json_set_i(json_constraint_range, L"quant", range->quant);
						json_set_name(json_constraint_range, L"values");
						json_push_back(json_scanner_option, json_constraint_range);
					}
						break;
					default:
						break;
